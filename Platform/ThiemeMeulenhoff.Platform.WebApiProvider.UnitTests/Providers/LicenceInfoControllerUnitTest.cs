...
public class LicenceInfoControllerUnitTest : BaseControllerUnitTest<LicenceInfo, ILicenceInfoLogicProvider, LicenceInfoController>
{
    #region [ CTor ]
    public LicenceInfoControllerUnitTest() {

    }
    #endregion

    #region [ Protected Methods - Override ]
    protected override LicenceInfoController OnGetController(ILogger<LicenceInfoController> logger, ILicenceInfoLogicProvider logic) {
        return new LicenceInfoController(logger, logic);
    }
    #endregion
    ...
    public async Task GetByProductIdAsync_Should_ReturnOk_If_Success() {
        ...
        var entity = this._fixture.Create<LicenceInfo>();
    ...
    public async Task GetByProductIdAsync_Should_ReturnNotFound_If_NotFound() {
        // Arrange
        var productId = this._fixture.Create<string>();
        this._logic.Setup(x => x.GetByProductIdAsync(productId)).ReturnsAsync(default(LicenceInfo));
        // Act
        var actual = await this._controller.GetByProductIdAsync(productId);
